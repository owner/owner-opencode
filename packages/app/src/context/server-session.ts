...
export function createServerSession(client: OpencodeClient, options?: { retry?: typeof retry }) {
  ...
  const loadMessages = async (sessionID: string, limit: number, before?: string, mode?: "replace" | "prepend") => {
    ...
    try {
      ...
      // Release the loading flag whenever this load still owns the slot, even if the
      // generation changed mid-load. Gating solely on the generation left `loading`
      // stuck true after a generation change, so every later load (including a forced
      // one) no-oped at the `meta.loading` guard and the transcript stayed blank.
