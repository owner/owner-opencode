...
describe("server session", () => {
  ...
  test("keeps the viewed transcript filled when a slow initial load lands after a fast prefetch", async () => {
    // Forge deep-link repro: the background limit=200 prefetch lands first with the full
    // transcript, then the throttled (3G) initial limit=2 load resolves afterwards. The
    // late, smaller page must not blank or clobber what the prefetch already committed.
    const user = userMessage("message-1")
    const assistant = assistantMessage("message-2", user.id)
    const slowInitial = deferredResponse()
    const client = {
      session: {
        get: async () => ({ data: session("child", "root") }),
        messages: (input: { limit: number }) =>
          input.limit === 200
            ? response([{ info: assistant, parts: [] }], "older")
            : slowInitial.promise,
        message: async () => singleResponse(user),
      },
    } as unknown as OpencodeClient
    const store = createServerSession(client)

    await store.prefetch("child", 200)
    expect(store.data.message.child).toEqual([user, assistant])

    const initial = store.sync("child", { force: true })
    slowInitial.resolve(response([{ info: assistant, parts: [] }], "older"))
    await initial
